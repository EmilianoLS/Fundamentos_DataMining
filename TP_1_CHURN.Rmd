...
Cargo dataframe de un tama√±o del 10% del original
...
Para eso creo una funci√≥n que me trae el porcentaje de nulos en las columnas que unicamente tienen nulos
...
Hago una anal√?tica sobre algunas variables para entender su relaci√≥n con la target a predecir

```{r}
ggplot(train) + geom_bar(mapping = aes(x = TutorialFinish, fill = churn), position = 'fill') +
labs(x = 'Termino el tutorial', y = '% churn') +
ggtitle('Proporcion de churn seg√∫n termin√≥ o no el tutorial')
```

Me imagino que una persona har√?a Churn seg√∫n haya perdido m√°s batallas en los primeros d√?as. Pongo a prueba esa suposici√≥n creando la sumatoria de las variables Lose battle, Win battle y Start battle
...
Podr√?a ocurrir que haya alguna relaci√≥n entre la cantidad de sesiones iniciadas y el hecho de hacer churn. Se busca la cantidad de sesiones iniciadas para los primeros 4 d√?as desde la instalaci√≥n hasta el d√?a 3, para ambos grupos.

Luego se calcula la media de sesiones iniciadas para entender si verdaderamente hay una relaci√≥n
...
ggtitle('Cantidad de sesiones por d√?a')
```
...
ggtitle('Cantidad de sesiones promedio por d√?a')
```

Quiero entender la cantidad de instalaciones por d√?a que hay, para ver si hay alguna tendencia
...
```{r}
descrCor <- cor(select_if(train_data_tr,is.numeric))
...
boxplot(train_data_tr[,c('EnterDeck_sum_dsi0','OpenChest_sum_dsi0','soft_negative','WinBattle_sum_dsi0','StartBattle_sum_dsi1')], col = 'orange', main = 'Features Boxplot - Top 5 variables', horizontal = TRUE)

```

La variable soft_negative es la que tiene outliers mas groseros, por lo que esta distorsionando la escala y no pueden verse las otras variables.

```{r}

options(repr.plot.width = 50, repr.plot.height = 25)
boxplot(train_data_tr[,c('EnterDeck_sum_dsi0','OpenChest_sum_dsi0','WinBattle_sum_dsi0','StartBattle_sum_dsi1')], col = 'orange', main = 'Features Boxplot - Top 5 variables', horizontal = TRUE, las = 1, ylab = 'Features')
...
```{r}
Q3 <- quantile(train_data_tr$EnterDeck_sum_dsi0, 0.75)
Q1 <- quantile(train_data_tr$EnterDeck_sum_dsi0, 0.25)

thd <- 1.5 * IQR(train_data_tr$EnterDeck_sum_dsi0)

limit_sup <- Q3 + thd
limit_inf <- Q1 - thd


train_data_tr <- filter(train_data_tr,(EnterDeck_sum_dsi0 < limit_sup)|(EnterDeck_sum_dsi0 > limit_inf))

```

```{r}
Q3 <- quantile(train_data_tr$OpenChest_sum_dsi0, 0.75)
Q1 <- quantile(train_data_tr$OpenChest_sum_dsi0, 0.25)

thd <- 1.5 * IQR(train_data_tr$OpenChest_sum_dsi0)

limit_sup <- Q3 + thd
limit_inf <- Q1 - thd


train_data_tr <- filter(train_data_tr,(OpenChest_sum_dsi0 < limit_sup)|(OpenChest_sum_dsi0 > limit_inf))
```


```{r}

Q3 <- quantile(train_data_tr$WinBattle_sum_dsi0, 0.75)
Q1 <- quantile(train_data_tr$WinBattle_sum_dsi0, 0.25)

thd <- 1.5 * IQR(train_data_tr$WinBattle_sum_dsi0)

limit_sup <- Q3 + thd
limit_inf <- Q1 - thd


train_data_tr <- filter(train_data_tr,(WinBattle_sum_dsi0 < limit_sup)&(WinBattle_sum_dsi0 > limit_inf))

```

```{r}

Q3 <- quantile(train_data_tr$soft_negative, 0.75)
Q1 <- quantile(train_data_tr$soft_negative, 0.25)

thd <- 1.5 * IQR(train_data_tr$soft_negative)

limit_sup <- Q3 + thd
limit_inf <- Q1 - thd


train_data_tr <- filter(train_data_tr,(soft_negative < limit_sup)&(soft_negative > limit_inf))
...
```{r}
#train_data_tr$Label <- factor(if_else(train_data_tr$Label == 1,'churn','no_churn')
...
xgb_train_1 = train(
  x = as.matrix(train_data_tr %>%
                  select(-Label)),
  y = as.factor(train_data_tr$Label),
