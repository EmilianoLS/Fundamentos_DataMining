...
train_data <- load_csv_data('train_4.csv')
...
set.seed(123)
val_index <- sample(c(1:nrow(train_filtered)), 217000)
...
rpart_fit_grid <- train(x = train_data_tr %>% select(-Label),
...
rpart_fit_grid <- train(x = validation_data %>% select(-Label),
                        y = validation_data$Label,
                        method = "rpart",
                        trControl = fitControl,
                        na.action = "na.omit",
